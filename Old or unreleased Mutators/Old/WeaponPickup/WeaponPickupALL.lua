...
function WeaponPickupALL:Start()
    ...
    self.showWeaponNames = self.script.mutator.GetConfigurationBool("showWeaponNames")
    ...
    self.isShowingWeaponName = false
    print("ShowWeaponNames:" .. tostring(self.showWeaponNames))

    self.isHitBoxInvisible = false
    if self.script.mutator.GetConfigurationBool("IsCubeInvisible") then
        self.isHitBoxInvisible = true
    end
...
function WeaponPickupALL:ActorDied(actor, killer, isSilent)
    ...
    if self.script.mutator.GetConfigurationBool("everyBotDrops") then
        if not isSilent and math.random() < (self.script.mutator.GetConfigurationRange("dropChance") / 100) then
            if
                Player.actor.activeWeapon ~= nil and Player.actor.activeWeapon.weaponEntry.name ~= nil and
                    actor.activeWeapon ~= nil
             then
                if
                    actor.activeWeapon.weaponEntry == nil or
                        actor.activeWeapon.weaponEntry.name == nil and Player.actor.activeWeapon.weaponEntry.name
                 then
                    return
                end
    ...
    if
        killer.name == Player.actor.name and not isSilent and
            math.random() < (self.script.mutator.GetConfigurationRange("dropChance") / 100)
     then
        if
            Player.actor.activeWeapon ~= nil and Player.actor.activeWeapon.weaponEntry.name ~= nil and
                actor.activeWeapon ~= nil
...
function WeaponPickupALL:IsLessOrEqual(localV, other)
    if (localV.x <= other.x and localV.y <= other.y and localV.z <= other.z) then
        return true
    else
        return false
...
function WeaponPickupALL:SpawnWeaponRaycast(selectedWeapon, actor)
    return function()
        ...
        if raycast ~= nil then
            ...
            spawnPos =
                actor.GetHumanoidTransformRagdoll(HumanBodyBones.Chest).position +
                Vector3(Random.Range(0, 0.7), 0, Random.Range(0, 0.7))
        end
        local randomRot = Random.Range(0.1, 0.7)
        local weaponToSpawn = selectedWeapon.InstantiateImposter(spawnPos, Quaternion(1, 1, 0, 0))
        local weaponColliderGameObject
        if self.isHitBoxInvisible then
            weaponColliderGameObject = GameObject.Instantiate(self.targets.weaponColliderInv)
        else
            weaponColliderGameObject = GameObject.Instantiate(self.targets.weaponCollider)
        end
        ...
        if weaponToSpawn.gameObject.GetComponent(Renderer) == nil then -- Because some modded weapons are lazily designed
            ...
            if
                self:IsGreaterOrEqual(
                    weaponToSpawn.gameObject.GetComponent(Renderer).bounds.size,
                    Vector3(1.9, 1.9, 1.9)
                )
             then
            ...
            elseif
                self:IsLessOrEqual(weaponToSpawn.gameObject.GetComponent(Renderer).bounds.size, Vector3(0.2, 0.2, 0.2))
             then
                weaponColliderGameObject.transform.localScale = Vector3(0.4, 0.4, 0.4)
            else
                weaponColliderGameObject.transform.localScale =
                    (weaponToSpawn.gameObject.GetComponent(Renderer).bounds.size + Vector3(0.11, 0.11, 0.11))
            end
        end

        -- weaponToSpawn.transform.localRotation = weaponToSpawn.transform.localRotation * Quaternion.AngleAxis(Random.Range(0,360), Vector3.up)
        ...
        self.script.StartCoroutine(
            self:DestroyWithDelay(weaponToSpawn, self.script.mutator.GetConfigurationInt("despawnTime"))
        )
    end
end
function WeaponPickupALL:SpawnWeaponDistance(selectedWeapon, actor)
    return function()
        ...
        --	print("Added " .. selectedWeapon.name .. " as y[1] and " .. weaponToSpawn.gameObject.name .. " for y[2]")
        self.script.StartCoroutine(
            self:Rotate(weaponToSpawn, 20, 0.021, self.script.mutator.GetConfigurationInt("despawnTime") - 1)
        )
        self.script.StartCoroutine(
            self:DestroyWithDelay(weaponToSpawn, self.script.mutator.GetConfigurationInt("despawnTime"))
        )
    ...
    return function()
        ...
        while (t < duration) and gameObject ~= nil do
            ...
            gameObject.transform.position =
                Vector3(gameObject.transform.position.x, yTransform, gameObject.transform.position.z)
...
function WeaponPickupALL:SetText(name)
    self.text.text = name
end
function WeaponPickupALL:Update()
    if (self.showWeaponNames) then
        local ray =
            Ray(
        ...
        if raycast ~= nil then
            if string.find(raycast.transform.gameObject.name, "SpawnedWeapon") then
                if (not self.text.gameObject.activeSelf) then
                ...
                self:SetText(weaponApply.name)
            end
        else
            if (self.text.gameObject.activeSelf) then
                self.text.gameObject.SetActive(false)
            end
        end
    end
