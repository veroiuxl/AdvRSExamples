...
function StealthAssist:Start()
		...
		if(self.script.mutator.GetConfigurationBool("isUsingCustomColor")) then
			...
			if(self:stringstartswith(hexColorString)) then
				local r,g,b = self:hex2rgb(hexColorString)
				self.customColor = Color(r,g,b,0)
			end
		end
		print(tostring(self.customColor))
		...
		self.spottedTime = self.script.mutator.GetConfigurationInt("spottedTime")
...
function StealthAssist:MuteFootsteps()
	self.footStepsSoundSources = GameObject.Find("Footsteps Sound Sources") 
	self.defaultFootStepsSoundVolume = {}
	self.footstepAudioSources = {}
	for i,y in ipairs(self.footStepsSoundSources.GetComponentsInChildren(AudioSource)) do
		table.insert(self.defaultFootStepsSoundVolume, y.volume)
	...
	for i,y in ipairs(self.footstepAudioSources) do
		y.volume = 0
		print(y.volume)
	end
...
function StealthAssist:UnMuteFootsteps()
	for i,y in ipairs(self.footstepAudioSources) do
		y.volume = self.defaultFootStepsSoundVolume[i]
	end
...
if(actor == Player.actor) then
	...
	if(self:contains(self.affectedBots,actor)) then
		self:RemoveOutline(actor)
	end
end
...
return function()
	...
	local botTexture = material.mainTexture
	bot.SetSkin(nil, {self.materialOutlineRed},1)
	material.SetColor("_OutlineColor",self.customColor)
	material.mainTexture = botTexture
...
function StealthAssist:SpotFOV()
	local actorsInRange = ActorManager.AliveActorsInRange(Player.actor.position,2800)
