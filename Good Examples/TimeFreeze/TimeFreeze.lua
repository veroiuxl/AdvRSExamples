...
function TimeFreeze:ToggleTime()
	...
	coroutine.yield(WaitForSeconds(0.002))
	self.isFrozen = true
	if(self.enableGoBackToPos) then
...
function TimeFreeze:Update()
	...
	if(Input.GetKey(KeyCode.LeftControl) and Input.GetKeyDown(KeyCode.H)) then
		if(self.enableGoBackToPos) then 
			self.enableGoBackToPos = false
	...
	if Input.GetKeyDown(KeyCode.H) and not Player.actor.isSeated then
		self.script.StartCoroutine("ToggleTime")
	end
